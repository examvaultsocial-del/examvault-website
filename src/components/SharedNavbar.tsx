  ...
  return (
    <nav className="w-full py-2 px-6 md:px-12 lg:px-20 flex items-center justify-between relative z-50">
      ...
      <div className="hidden lg:flex items-center gap-8">
        {navLinks.map((link) => {
          if (link.hasDropdown) {
            return (
              ...
              >
                ...
                {/* Sketchy underline for hover */}
                <div className="absolute bottom-0 left-0 w-0 group-hover:w-full transition-all duration-300 overflow-hidden">
                  <svg width="100%" height="8" viewBox="0 0 100 8" preserveAspectRatio="none" className="text-[#2D2D2D] opacity-40">
                    <path d="M0,6 Q25,2 50,6 T100,6" stroke="currentColor" strokeWidth="1.5" fill="none" strokeLinecap="round" />
                  </svg>
                </div>

                {/* Beautiful Dropdown Card */}
                <div className="absolute top-[100%] left-1/2 -translate-x-1/2 mt-1 w-64 bg-white border-2 border-[#E5E5E5] rounded-2xl shadow-xl opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-all duration-300 z-50 p-2 flex flex-col gap-1">
          ...
          return (
            ...
            >
              ...
              {/* Sketchy Underline for Active/Hover */}
              {link.active ? (
                <svg width="100%" height="8" viewBox="0 0 100 8" preserveAspectRatio="none" className="absolute bottom-0 left-0 text-[#B59410] opacity-90 pointer-events-none" style={{ filter: 'url(#pencilFilter)' }}>
                  <path d="M0,6 Q25,2 50,6 T100,6" stroke="currentColor" strokeWidth="1.5" fill="none" strokeLinecap="round" />
                </svg>
              ) : (
                <div className="absolute bottom-0 left-0 w-0 group-hover:w-full transition-all duration-300 overflow-hidden">
        ...
        <button
          onClick={onCartClick}
          className="relative p-2.5 rounded-full border-2 border-[#2D2D2D]/15 hover:border-[#2D2D2D] bg-[#FFFDF9] transition-all hover:scale-105 hover:rotate-3 duration-200 cursor-pointer shadow-sm group mr-2"
      ...
      <div className="lg:hidden flex items-center gap-3">
        ...
        >
          ...
          {cartCount > 0 && (
            <span 
              className="absolute -top-1.5 -right-1.5 bg-[#FCD34D] text-[#2D2D2D] border-2 border-[#2D2D2D] font-extrabold text-[9px] w-5.5 h-5.5 flex items-center justify-center rounded-full shadow-sm animate-bounce"
              style={{ animationDuration: "2.5s" }}
            >
              {cartCount}
            </span>
        ...
        >
          <div className="flex flex-col gap-6">
            ...
            <div className="flex items-center justify-between border-b-2 border-[#2D2D2D]/10 pb-4">
              <span className="font-sketch text-3xl text-[#2D2D2D]">
                {menuTitle}
              </span>
              <button 
                onClick={() => setMobileMenuOpen(false)}
                  ...
                  return (
                    <div key={link.name} className="flex flex-col">
                      <button
                        ...
                        className="flex items-center justify-between py-2 text-lg font-bold text-[#4A4A4A] hover:text-[#B59410] transition-colors text-left cursor-pointer"
                      >
                        <span>{link.name}</span>
                        {mobileDropdownOpen ? (
                      ...
                      <div 
                        className={`pl-4 flex flex-col gap-1.5 overflow-hidden transition-all duration-300 ${
                          mobileDropdownOpen ? "max-h-[300px] mt-2 mb-1 opacity-100" : "max-h-0 opacity-0"
                        }`}
                      >
                        {dropdownItems.map((item) => (
                          <Link
